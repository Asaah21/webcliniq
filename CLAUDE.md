...
The streaming migration is done (September 2026).

**`netlify/edge-functions/audit.js` is the audit engine**: Places, PageSpeed, website content
checks, Gemini enrichment, scoring and the `audits` insert. It is a Deno edge function served at
`/api/audit-stream` (wired in `netlify.toml`) and streams Server-Sent Events to `script.js`.

**`netlify/functions/audit.js` is email capture only** (`action: 'capture_email'`): it emails the
already-computed results via Resend, inserts into `audit_leads`, and notifies the owner. It runs
no checks. `netlify/functions/report.js` serves the read-only `/r/<public_id>` report page.

## Stack
- Frontend: Vanilla JS, plain HTML/CSS — no frameworks
- Functions: Netlify Functions (Node.js) — email capture (`audit.js`) and the report page (`report.js`)
- Edge Functions: Netlify Edge Functions (Deno) — the streaming audit engine
- Database: Supabase
- Email: Resend
...
| File | Job |
|---|---|
| `netlify/edge-functions/audit.js` | Streaming audit engine (Deno) at `/api/audit-stream` |
| `netlify/functions/audit.js` | Email capture only (Node) |
| `netlify/functions/report.js` | `/r/<public_id>` report page (Node) |
| `script.js` | Frontend audit UI and stream reader, hero demo loop |
| `styles.css` | All styling |
| `*.html` | Structure and copy only |

New checks, scoring changes and enrichment work all go in `netlify/edge-functions/audit.js`.

## Copy rules
- First person singular ("I", "me") on marketing pages. Automated audit output stays neutral
  ("Checked yoursite.com", not "I audited"). Privacy and Terms use "we" for the business.
- No em dashes in anything a visitor sees. Gemini output is stripped of them in `enrichWithAI`.
- No invented statistics or claims about competitors the audit didn't measure.
...
- The edge function emits these stream events: searching, match, listing, website, pagespeed, enrichment, complete, error
...
## Deno vs Node — `netlify/edge-functions/audit.js` only
...
- Do not add audit logic back into `netlify/functions/audit.js`; it is email capture only

## Local dev gotchas
- `netlify dev` shares the production Supabase database; every local audit is a real row.
- `netlify dev` can crash with `ENOENT ... audit.js.tmp.*` when an editor saves
  `netlify/edge-functions/audit.js`; restart it.
